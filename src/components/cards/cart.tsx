...
const CartCard: FC<CartCardType> = ({ collection, name, path, price }) => {
    ...
    <div className={cn("w-full flex items-center flex-col md:flex-row border-2 border-nav")}>
      ...
      <div className="flex w-full justify-between md:justify-end px-5 md:px-0 md:pr-10 space-x-5 text-card-text">
        <div className="flex md:border-r-2 h-[50px] border-nav pr-10 items-center space-x-2">
          <span onClick={addCounter}>
            {" "}
            <BsFillPlusSquareFill size={25} />{" "}
          </span>
          <span className="bg-nav px-3 py-1 text-coral font-bold rounded">{counter} </span>
          <span onClick={redCounter}>
            {" "}
            <BsFillDashSquareFill size={25} />
          </span>
        </div>
        <div className=" flex items-center">
          <span className="border-nav border p-2">
            {" "}
            <BsTrash3 size={25} />
          </span>
        </div>
